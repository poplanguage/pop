...
fn assert_success_feedback(stderr: &[u8], phase: &str, command: &str) {
    let stderr = output_text(stderr);
    assert!(stderr.contains(phase), "{stderr}");
    assert!(stderr.contains("[1/1]"), "{stderr}");
    assert!(
        stderr.contains(&format!("Finished  `{command}`")),
        "{stderr}"
    );
}
...
fn check_dumps_deterministic_verified_hir_for_a_pop_module() {
    ...
    assert_success_feedback(&first.stderr, "Checking", "check");
}
...
fn check_accepts_repeatable_dump_options_in_command_line_order() {
    ...
    assert_success_feedback(&output.stderr, "Checking", "check");
...
fn invalid_source_emits_a_structured_diagnostic_and_no_dump() {
    ...
    assert!(stderr.contains("invalid.pop:"), "{stderr}");
    assert!(stderr.contains("return missingValue"), "{stderr}");
...
fn transpile_to_c_is_deterministic_and_emits_a_complete_translation_unit() {
    ...
    assert_success_feedback(&first.stderr, "Transpiling", "transpile");
...
fn transpile_supports_the_runtime_free_native_math_example() {
    ...
    assert_success_feedback(&output.stderr, "Transpiling", "transpile");
